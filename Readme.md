...
Trong source code có chứa file `.env` để config, trong file này ta có thể đổi port cho API backend, mặc định là port `4000`
...
Đối với các API cần xác thực người dùng như bên cụm API về `Account` thì ta cần gửi accessToken lên server thông qua header `Authorization: "Bearer <accessToken>"`
